    ...
    <el-form :model="billInStockFlow" :rules="rules" ref="billInStockFlow" label-width="110px">
      ...
      <div class="section">
        ...
        <div class="newBtn" v-else>
          <el-button @click="handleOut">下一步</el-button>
          <el-button @click="handleBack">取消</el-button>
        </div>
      </div>
    </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        openAccountBank: [{ required: true, message: "请输入正确开户行" }], //开户行
        contactPerson: [{ required: true, message: "请输入正确联系人" }], //联系人
        bigLineNo: [{ required: true, message: "请输入正确行号" }], //大额行号
        ...
        ticketNo: [{ required: true, message: "请输入正确票号" }], //票号
        applyUserName: [{ required: true, message: "请输入正确名字" }], //申请人
        edit_status:1,
        opponentCompany: [
          {
            required: true,
  ...
  methods: {
    ...
    handleOut(){
      this.$router.push('/ModifyMoveOutLibrary');
    },
    // 对手类型
      ...
      if (row.isSet) {
       ...
       let data = JSON.parse(JSON.stringify(this.master_user.sel));
        ...
        if (data.outStockInterest) {
          data.receiveMoney = 0;
          data.receiveMoney = (data.ticketAmount - data.outStockInterest-data.outEleRemitCost).toFixed(2)-0;
        }
        data.holderCost =
    ...
    billOutStockFlow_(param) {
      billOutStockFlow_(param).then(res => {
        if (res && res.data && res.data.CODE == "200") {
          ...
          let datas = result.moveFlow;
          sessionStorage.setItem('moveOutFlow_',JSON.stringify(result))
...
.modifyMoveLibrary header > div {
  width: 33%;
}
.modifyMoveLibrary header > div:last-child {
  width: 34%;
}
.modifyMoveLibrary header div > label {
  display: block;
  margin-bottom: 24px;
}
.modifyMoveLibrary header div > label > span {
  margin-right: 10px;
  font-weight: 600;
}
.modifyMoveLibrary .infoHead {
  border-bottom: 2px solid #e6e9f0;
}
.modifyMoveLibrary .infoHead div {
  padding-bottom: 8px;
}
.modifyMoveLibrary .infoHead .fl {
  padding-left: 12px;
  position: relative;
  color: #5376ff;
}
.modifyMoveLibrary .infoHead .fl i {
  position: absolute;
  left: 0;
  bottom: -2px;
  width: 80px;
...
.modifyMoveLibrary .info {
  margin-bottom: 25px;
}
.modifyMoveLibrary .section .headClass {
  color: #a3adc8;
}
.modifyMoveLibrary .section .addRow {
  margin-top: 25px;
  color: #5376ff;
  display: inline-block;
  cursor: pointer;
}
.modifyMoveLibrary .section .addRow i {
  margin-right: 8px;
}
.modifyMoveLibrary .section .total {
  margin-top: 27px;
}
.modifyMoveLibrary .section .total .top {
  margin-bottom: 16px;
  color: #596689;
}
.modifyMoveLibrary .section .total li {
...
.modifyMoveLibrary .section .total li .txt {
  line-height: 26px;
  font-size: 12px;
  color: rgba(163, 173, 200, 1);
}
.modifyMoveLibrary .section .total li .num {
  line-height: 26px;
  font-size: 18px;
  font-weight: 600;
