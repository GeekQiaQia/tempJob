<template>
  <div class="ModifyMoveOutLibrary">
    <el-form :model="billInStockFlow" :rules="rules" ref="billInStockFlow" label-width="110px">
      <header class="clearfix">
        <div class="fl">
          ...
          <el-form-item label="打款方式：" prop="payType">
            <el-select v-model="billInStockFlow.payType" placeholder="请选择" class="width_120">
              <el-option
        ...
        <div class="fl">
          <el-form-item label="出库渠道：" prop="outChannelType">
            <el-select v-model="billInStockFlow.outChannelType" placeholder="请选择" class="width_120">
              <el-option
        ...
        <div class="fl">
          <el-form-item label="对手类型：" prop="opponentType">
            <el-select v-model="billInStockFlow.opponentType" placeholder="请选择" class="width_120" @change="handleOpponentType">
              <el-option
          ...
          <div class="clearfix" style="min-width:392px">
            ...
            <el-form-item label prop="contactPhone" class="fl library">
              <el-input v-model="billInStockFlow.contactPhone" class="input88" placeholder="联系电话"></el-input>
            </el-form-item>
      ...
      <header class="clearfix">
        <ul class="libraryUl">
          ...
          <li v-for="(item,index) in applicantList">
      ...
      <header class="clearfix">
        <div class="fl">
          <el-form-item label="备注：" prop="remark">
            <el-input v-model="billInStockFlow.remark" class="width_280" placeholder="请输入姓名"></el-input>
          </el-form-item>
        </div>
      </header>
      <nav class="info">
      ...
      <div class="section">
        ...
        >
          ...
          <el-table-column width="60" fixed="right" v-if="master_user.data.length >0" style="display:none">
            <template slot-scope="scope">
              <span
        ...
        <div class="newBtn" v-if="auditStatus==50000">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        openAccountBank: [{ required: true, message: "请输入开户行" }], //开户行
        contactPerson: [{ required: true, message: "请输入联系人" }], //联系人
        bigLineNo: [{ required: true, message: "请输入行号" }], //大额行号
        // accountNo: [{ required: true, message: "请输入账号" }], //账号
        accountNo: [
          { required: true, validator: this.checkData.check_zero_digit}
        ], //账号
  ...
  methods: {
        ...
        if (res && res.data && res.data.CODE == "200") {
          ...
          if (res.data.RESULT.length == 0) {
            if(!inx){
              ...
              applicantList && applicantList.map(function(v,i) {
                if(inx-1==i) {
                    applicantList[i]={
                        applyUserId: '',
                        applyDeptId: '',
    ...
    submitForm(formName) {
      ...
      billInStockFlow.flowType = 30002;
      ...
      billInStockFlow.ticketList = ticketData;
          ...
          billOutStockFlow_save(billInStockFlow).then(res => {
            if (res && res.data && res.data.CODE == "200") {
              ...
              billInStockFlow.moveFlowId = res.data.RESULT.moveFlowId;
              // sessionStorage.setItem(
              //   "inventoryWarehouse",
              //   JSON.stringify(billInStockFlow)
              // );
              this.$router.push("/outgoingApplication");
            } else {
                
                 billInStockFlow.ticketList &&
                  billInStockFlow.ticketList.map(function(v, i) {
    ...
    pwdChange(row, index, cg) {
      ...
      if (row.isSet) {
        ...
        if (data["outCalInterestType"] == "元/每十万") {
          ...
          data.outStockInterest = ((data.ticketAmount - 0) / 100000).toFixed(2)-0;
        } else {
          data.outStockInterest = 0;
        ...
        if (data.outStockInterest) {
          data.receiveMoney = 0;
          data.receiveMoney = (data.ticketAmount - data.outStockInterest).toFixed(2)-0;
        }
        data.holderCost =
          (((data.ticketAmount - 0) * (data.holderTicketDays - 0) * 6) / 36000).toFixed(2)-0;
        // 收款-放款-成本
        data.earn = (data.receiveMoney - data.payMoney - data.holderCost).toFixed(2)-0;
        ...
        datas &&
          datas.map(function(v, i) {
            if (row.ticketNo == v.ticketNo) {
               v.receiveMoney = (data.receiveMoney).toFixed(2)-0;
              v.outStockInterest = (data.outStockInterest).toFixed(2)-0;
            }
            if(v.receiveMoney){
    ...
    billOutStockFlow_() {
          let result=JSON.parse(sessionStorage.getItem('moveOutFlow_'));
          let datas = result.moveOutFlow;
          let ticketAmountTotal = 0;
          let inPayMoneyTotal = 0;
          let inInterestTotal = 0;
          let outReceiveMoneyTotal = 0;
          let outInterestTotal = 0;
          this.auditStatus= datas.auditStatus;
          datas.ticketList &&
          ...
          this.billInStockFlow = Object.assign(billInStockFlow, {
            ...
            outReceiveMoneyTotal,
            outInterestTotal,
            ticketType: datas.ticketList[0].ticketType,
            mediaType: datas.ticketList[0].mediaType,
          },datas,{
            moveFlowId:result.moveFlowId,
            moveOutFlowId:result.moveOutFlowId,
            parentFlowId:datas.parentFlowId,
          });
          this.master_user.data = datas.ticketList;
    }
  },

  created() {
    // this.billOutStockFlow_({ id: this.$route.query.id });
    this.billOutStockFlow_()
  }
};
</script>
...
.ModifyMoveOutLibrary {
  padding: 16px 32px;
}
.ModifyMoveOutLibrary header > div {
  width: 33%;
}
.ModifyMoveOutLibrary header > div:last-child {
  width: 34%;
}
.ModifyMoveOutLibrary header div > label {
  display: block;
  margin-bottom: 24px;
}
.ModifyMoveOutLibrary header div > label > span {
  margin-right: 10px;
  font-weight: 600;
}
.ModifyMoveOutLibrary .infoHead {
  border-bottom: 2px solid #e6e9f0;
}
.ModifyMoveOutLibrary .infoHead div {
  padding-bottom: 8px;
}
.ModifyMoveOutLibrary .infoHead .fl {
  padding-left: 12px;
  position: relative;
  color: #5376ff;
}
.ModifyMoveOutLibrary .infoHead .fl i {
  position: absolute;
  left: 0;
  bottom: -2px;
  width: 80px;
...
.ModifyMoveOutLibrary .info {
  margin-bottom: 25px;
}
.ModifyMoveOutLibrary .section .headClass {
  color: #a3adc8;
}
.ModifyMoveOutLibrary .section .addRow {
  margin-top: 25px;
  color: #5376ff;
  display: inline-block;
  cursor: pointer;
}
.ModifyMoveOutLibrary .section .addRow i {
  margin-right: 8px;
}
.ModifyMoveOutLibrary .section .total {
  margin-top: 27px;
}
.ModifyMoveOutLibrary .section .total .top {
  margin-bottom: 16px;
  color: #596689;
}
.ModifyMoveOutLibrary .section .total li {
...
.ModifyMoveOutLibrary .section .total li .txt {
  line-height: 26px;
  font-size: 12px;
  color: rgba(163, 173, 200, 1);
}
.ModifyMoveOutLibrary .section .total li .num {
  line-height: 26px;
  font-size: 18px;
  font-weight: 600;
