<template>
  <div class="outboundReview">
    ...
    <ul class="option clearfix">
        ...
        <p>
          ...
          <el-popover ref="popover1" placement="bottom" width="150" trigger="hover">
            <div style="padding-left:10px">
              ...
              >{{outBoundDatas.operaRecordList[0].passOperaContent.indexOf(item_.value) !=-1 ? item_.label :''}}</p>
            </div>
          </el-popover>
        </p>
        <p>
        ...
        <p>
          ...
          <el-popover ref="popover2" placement="bottom" width="150" trigger="hover">
            <div style="padding-left:10px">
              <p
                v-for="item_ in reviewOpinions"
              >{{outBoundDatas.operaRecordList[1].passOperaContent.indexOf(item_.value) !=-1 ? item_.label :''}}</p>
            </div>
          </el-popover>
        </p>
        <p>
      ...
      <li  v-else-if="(flowType==30004  && outBoundDatas.auditStatus ==50001 && jobType==1100 || flowType==30001  && outBoundDatas.auditStatus ==50001 && jobType==1100)">
        <div>复核意见</div>
        <p>
    ...
    <div class="newBtn" v-if="(flowType==30001 && jobType==1100 || status==1 && outBoundDatas.auditStatus==50001 && jobType==1100 || status==2 && outBoundDatas.auditStatus==50001 && jobType==1100) && !edit">
      <el-button @click="handle">确认</el-button>
      <el-button @click="handleBack">取消</el-button>
    </div>
    <div class="newBtn" v-else-if="((flowType==30004 && status==1 && jobType !=1100) || (flowType==30004 && status==1 && outBoundDatas.auditStatus-datas.auditStatus>0))">
...
header > ul:nth-child(2) li > div:nth-child(1) {
  width: 356px;
}
header ul > li {
  padding-left: 16px;
...
.section .total li .txt {
  line-height: 26px;
  font-size: 12px;
  color: rgba(163, 173, 200, 1);
}
.section .total li .num {
  line-height: 26px;
  font-size: 18px;
  font-weight: 500;
