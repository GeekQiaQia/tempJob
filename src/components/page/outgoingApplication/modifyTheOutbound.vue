      ...
      <header class="clearfix">
        <div class="fl">
          ...
          <el-form-item label="账号：" prop="accountNo">
            <el-input v-model.number="billInStockFlow.accountNo" class="width_280"></el-input>
          </el-form-item>
        </div>
        <div class="fl">
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let ticketData = this.master_user.data;
          ticketData &&
    ...
    pwdChange(row, index, cg) {
      ...
      if (data["outCalInterestType"] == "元/每十万") {
        ...
        // 收款-放款-成本
        data.earn =
          (data.receiveMoney - data.payMoney - data.holderCost).toFixed(2) - 0;
    ...
    billOutStockFlow_(param) {
      billOutStockFlow_(param).then(res => {
        if (res && res.data && res.data.CODE == "200") {
          ...
          let outReceiveMoneyTotal = 0;
          let outInterestTotal = 0;
          let ticketList = datas.ticketList;
          ticketList &&
          ...
          this.billInStockFlow = Object.assign(
            billInStockFlow,
            {
              ticketAmountTotal,
              inPayMoneyTotal,
              ...
              outReceiveMoneyTotal,
              outInterestTotal,
              ticketType: ticketList[0] && ticketList[0].ticketType,
              mediaType: ticketList[0] && ticketList[0].mediaType,
              auditStatus: 0
            },
            datas
          );
...
.modifyTheOutbound header > div {
  width: 33%;
}
.modifyTheOutbound header > div:last-child {
  width: 34%;
}
.modifyTheOutbound header div > label {
  display: block;
  margin-bottom: 24px;
}
.modifyTheOutbound header div > label > span {
  margin-right: 10px;
  font-weight: 600;
}
.modifyTheOutbound .infoHead {
  border-bottom: 2px solid #e6e9f0;
}
.modifyTheOutbound .infoHead div {
  padding-bottom: 8px;
}
.modifyTheOutbound .infoHead .fl {
  padding-left: 12px;
  position: relative;
  color: #5376ff;
}
.modifyTheOutbound .infoHead .fl i {
  position: absolute;
  left: 0;
  bottom: -2px;
  width: 80px;
...
.modifyTheOutbound .info {
  margin-bottom: 25px;
}
.modifyTheOutbound .section .headClass {
  color: #a3adc8;
}
.modifyTheOutbound .section .addRow {
  margin-top: 25px;
  color: #5376ff;
  display: inline-block;
  cursor: pointer;
}
.modifyTheOutbound .section .addRow i {
  margin-right: 8px;
}
.modifyTheOutbound .section .total {
  margin-top: 27px;
}
.modifyTheOutbound .section .total .top {
  margin-bottom: 16px;
  color: #596689;
}
.modifyTheOutbound .section .total li {
...
.modifyTheOutbound .section .total li .txt {
  line-height: 26px;
  font-size: 12px;
  color: rgba(163, 173, 200, 1);
}
.modifyTheOutbound .section .total li .num {
  line-height: 26px;
  font-size: 18px;
  font-weight: 600;
