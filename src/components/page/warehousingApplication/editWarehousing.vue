        ...
        <ul class="fl">
          ...
          <div class="clearfix">
              <el-form-item label="联系人：" prop="contactPerson"  class="fl">
                ...
                <el-input v-model="billInStockFlow.contactPhone" style="width:116px" placeholder="请输入电话" maxlength="11" class="input106"></el-input>
              </el-form-item>
          ...
          <el-form-item label="备注：" prop="remark">
            <el-input v-model="billInStockFlow.remark" class="input348"></el-input>
          </el-form-item>
        </ul>
        <ul class="fl">
          ...
          <el-form-item label="申请人：" prop="applyUserName">
            <el-input v-model="billInStockFlow.applyUserName" class="input106" disabled placeholder="请输入姓名"></el-input>
            <el-input v-model="billInStockFlow.departmentName" class="input106" disabled placeholder="请输入部门"></el-input>
          </el-form-item>
        </ul>
      </header>
      <nav class="info">
          ...
          >
            <template slot-scope="scope">
              <div>
                ...
                <span v-else-if="v.field=='calInterestType'">{{scope.row[v.field]=='13001' ? '年息%':'元/每十万'}}</span>
                ...
                <div class="table_info">
                      ...
                      <li>
                        ...
                        <p class="specil three">
                          <el-form-item label="开户银行" prop="drawerBank">
                            <el-input v-model="listItem.drawerBank" v-if="editStatus" placeholder="请输入开户银行" class="drawerBank"></el-input>
                  ...
                  <div class="guarantee_info">
                    <ul class="person clearfix">
                      ...
                      >票据金额</li>
                      <li class="guarantee_name_">
                        <el-form-item label="人民币（小写）" prop="ticketAmount">
                          <el-input v-model="listItem.ticketAmount" style="width:151px" placeholder="请输入人民币" v-if="editStatus" @blur="handleticketAmount(listItem.ticketAmount)"></el-input>
                          <div v-else  class="border_"  style="width:150px">{{listItem.ticketAmount}}</div>
                        </el-form-item>
                      </li>
                      <li
                        class="guarantee_name"
                    ...
                    <ul class="person clearfix">
                      ...
                      <li class="acceptor_">
                        <p class="specil acceptor_o">
                          ...
                          <div v-else  class="border_" style="width: 220px;">{{listItem.accepter}}</div>
                          </el-form-item>
                        </p>
  ...
  if (reg.test(value)) {
    ...
    return {
      ...
      rules: {
        ...
        openAccountBank: [{ required: true,validator:this.checkData.check_chinese, message: "请输入正确开户行" }], //开户行
        contactPerson: [{ required: true, message: "请输入正确联系人", validator:this.checkData.check_chinese }], //联系人
        bigLineNo: [{ required: true, message: "请输入正确大额行号" ,validator: this.checkData.check_zero_digit}], //大额行号
        accountNo: [
          { required: true, validator: this.checkData.check_zero_digit, trigger: "blur" , message: "请输入正确账号"}
        ], 
        ticketAmount: [{ required: true,validator:check_ticketAmount,trigger: "blur",message: "请输入正确票面金额"}], //票面金额
        contactPhone: [
          { required: true, validator: this.checkData.check_mobile, trigger: "blur" }
        ], //账号
        postName: [
          {  validator: this.checkData.check_chinese_not, trigger: "blur" ,message: "请输入正确职务"}
        ],// 职务
        ticketNo: [{ required: true,validator:check_ticketNo,trigger: "blur",message: "请输入正确票号"}], //票号
        eleRemitCost: [{ validator:check_eleRemitCost,trigger: "blur",message: "请输入正确电汇费"}], //电汇费
        draftDate: [{ required: true, message: "请选择日期" }], //日期
        expireDate: [{ required: true, message: "请选择日期" }], //日期
        drawer: [{ required: true, validator:this.checkData.check_chinese,message: "请输入正确全称" }], //出票人
        drawerAccountNo: [{  validator:check_maxAccountNum_,message: "请输入正确账号" }], //出票人账号
        drawerBank: [{  validator:this.checkData.check_chinese_not,message: "请正确开户行" }], //出票人银行
        receiverAccountNo: [{ validator:check_maxAccountNum_,message: "请输入正确账号" }], 
        receiverBank: [{ validator:this.checkData.check_chinese_not,message: "请输入正确开户行" }],// 收款人开户行 
        accepterAccountNo: [{ validator:check_maxAccountNum_,message: "请输入正确账号" }], //承兑人账号
        accepterBankNo: [{ required: true, validator:this.checkData.check_zero_digit,message: "请输入正确开户行行号" }], //承兑人开户行行号
        receiver: [{ required: true, validator:this.checkData.check_chinese,message: "请输入正确全称" }], //收款人 
        accepterBank: [{ validator:this.checkData.check_chinese_not,message: "请输入正确名称" }], //承兑人开户行名称
        accepter: [{required: true,validator:this.checkData.check_chinese,message: "请输入正确全称" }], //承兑人
        stockPrice: [{ required: true ,validator:check_stockPrice,trigger: "blur"}], //入库价格
      }
    };
  },
  methods: {
    ...
    handleChangeTicket(){
        if(this.listItem.calInterestType==13001){
          if((this.listItem.dayNum || this.listItem.dayNum==0) && this.listItem.stockPrice){
          ...
          if(this.listItem.ticketAmount){
            ...
            if(reg.test(stockPrice)){
              this.listItem.stockInterest  = ((this.listItem.ticketAmount - 0) / 100000  * (stockPrice-0)).toFixed(2)-0;
            }
          }
        }
        if(this.listItem.ticketAmount){
          ...
          //入库放款
          let payMoney=this.listItem.ticketAmount-stockInterest-eleRemitCost;
          if(payMoney<=0){
            payMoney=0
          }
    ...
    handleSubmit(formName){
      ...
      tableData && tableData.map(function(v,i) {
        ...
        let draftDate=JSON.parse(JSON.stringify(v.draftDate));
        let expireDate=JSON.parse(JSON.stringify(v.expireDate));

        if(draftDate.indexOf('-')>=0){
          let draftDate_a=draftDate.substr(0,4);
          let draftDate_b=draftDate.substr(5,2);
          let draftDate_c=draftDate.substr(8,2);
          v.draftDate=draftDate_a+''+draftDate_b+''+draftDate_c
        }
        if(expireDate.indexOf('-')>0){
          let expireDate_a=expireDate.substr(0,4);
          let expireDate_b=expireDate.substr(5,2);
          let expireDate_c=expireDate.substr(8,2);
          v.expireDate=expireDate_a+''+expireDate_b+''+expireDate_c
        }
      })
      if(!flag){
          this.$message({
    ...
    handleExpireDate(value){
      let dateTime=value && (value.substr(0,4)+value.substr(5,2)+value.substr(8,2));
      getDaysUrl({
        ...
        if (res && res.data && res.data.CODE == "200") {
          let result=res.data.RESULT;
           if(result && (result.plusDay || result.plusDay==0)){
              this.listItem.plusDay= result.plusDay;
              let day=Math.ceil((new Date(value)-new Date())/(3600*1000*24))+result.plusDay;
              if(day<0){
                this.listItem.dayNum=0
              }else {
                this.listItem.dayNum=day
              }
           }
        }
      })
    },
    handleEditItem(value,val){
      this._index=val
      this.dialogVisible=true;
      this.editStatus=2;
      // this.listItem=JSON.parse(JSON.stringify(value));
      let listItem=JSON.parse(JSON.stringify(value));
      let ticketAmount=listItem.ticketAmount && (parseFloat(listItem.ticketAmount)).toFixed(2);
    ...
    handleView(row){
      ...
      this.editStatus=0;
      let ticketAmount=this.listItem.ticketAmount && (parseFloat(this.listItem.ticketAmount)).toFixed(2)
      this.ticketAmount_big=this.smalltoBIG(ticketAmount)
    },
    ...
    getBussinessDetail(param) {
      getBussinessDetail(param).then(res => {
       if (res && res.data && res.data.CODE == "200") {
           let billInStockFlow=res.data.RESULT;
            sessionStorage.setItem('wareEditDatas',JSON.stringify(billInStockFlow))
           let ticketList=res.data.RESULT.ticketList;
            let ticketAmountTotal=0;
            let inPayMoneyTotal=0;
            this.data=ticketList;
            billInStockFlow.payType=billInStockFlow.payType+'';
            billInStockFlow.inStockType=billInStockFlow.inStockType+''
           this.billInStockFlow=billInStockFlow;
           
           ticketList && ticketList.map(function(v,i) {
               v.expireDate=v.expireDate+'';
               v.draftDate=v.draftDate+''
               v.draftDate=v.draftDate+''
                 if(v.ticketAmount){
                    ticketAmountTotal+=(v.ticketAmount-0);
                }
        ...
        this.inPayMoneyTotal=inPayMoneyTotal;
        }else {
            this.$message({
            message: res.data.MESSAGE,
            type: "warning"
...
.editWarehousing header > ul:nth-child(1) {
  margin-right: 174px;
  /* width: 50%; */
}
.editWarehousing header ul > label {
  display: block;
  margin-bottom: 24px;
}
.editWarehousing header ul > label > span {
  margin-right: 10px;
  font-weight: 600;
}
.editWarehousing .infoHead {
  border-bottom: 2px solid #e6e9f0;
}
.editWarehousing .infoHead div {
  padding-bottom: 8px;
}
.editWarehousing .infoHead .fl {
  padding-left: 12px;
  position: relative;
  color: #5376ff;
}
.editWarehousing .infoHead .fl i {
  position: absolute;
  left: 0;
  bottom: -2px;
  width: 80px;
...
.editWarehousing .info {
  margin-bottom: 25px;
}
.editWarehousing .section .headClass {
  color: #a3adc8;
}
.editWarehousing .section .addRow {
  margin-top: 25px;
  color: #5376ff;
  display: inline-block;
  cursor: pointer;
}
.editWarehousing .section .addRow i,.editWarehousing .section .total i {
  vertical-align:middle;
  margin-right: 8px;
}
.editWarehousing .section .total {
  margin-top: 27px;
}
.editWarehousing .section .total .top {
  margin-bottom: 16px;
  color: #596689;
}
.editWarehousing .section .total li {
...
.editWarehousing .section .total li .txt {
  line-height: 26px;
  font-size: 12px;
  color: rgba(163, 173, 200, 1);
}
.editWarehousing .section .total li .num {
  line-height: 26px;
  font-size: 18px;
  font-weight: 600;
