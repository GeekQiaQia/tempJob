  ...
  <div class="newMoveLibrary">
    <el-form :model="billInStockFlow" :rules="rules" ref="billInStockFlow" label-width="110px">
      <header class="clearfix">
        ...
        <div class="fl">
          <el-form-item label="出库渠道：" prop="outChannelType">
            <el-select v-model="billInStockFlow.outChannelType" placeholder="请选择" class="width_120"  @change="changeoutChannelType">
              <el-option
        ...
        <div class="fl">
          ...
          <div class="clearfix" style="min-width:414px">
      ...
      <div class="section">
        ...
        <div class="newBtn">
          <el-button @click="submitForm('billInStockFlow')">下一步</el-button>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      //   ], //收款企业
      //   inStockType: [{ required: true, message: "请选择入库方式" }], //入库方式
      //   openAccountBank: [{ required: true,validator: this.checkData.check_chinese, message: "请输入开户行" }], //开户行
      //   contactPerson: [{ required: true, message: "请输入联系人" }], //联系人
      //   bigLineNo: [{ required: true, message: "请输入行号" }], //大额行号
      //   // accountNo: [{ required: true, message: "请输入账号" }], //账号
      //   accountNo: [
      //     { required: true, validator: this.checkData.check_zero_digit, trigger: "blur" }
      //   ], //账号
      //   ticketNo: [{ required: true, message: "请输入票号" }], //票号
      //   applyUserName: [{ required: true, message: "请输入名字" }], //申请人
      //   opponentCompany: [
  ...
  methods: {
        ...
        if (valid) {
          ...
          billOutStockFlow_save(billInStockFlow).then(res => {
            if (res && res.data && res.data.CODE == "200") {
              ...
              this.$router.push("/inventoryWarehouse");
            } else {
    ...
    pwdChange(row, index, cg) {
      ...
      if (row.isSet) {
        ...
        if (data["outCalInterestType"] == "元/每十万") {
          ...
          data.outStockInterest = ((data.ticketAmount - 0) / 100000 *(data.outStockPrice)).toFixed(2);
        ...
        datas &&
          ...
          outReceiveMoneyTotal=outReceiveMoneyTotal && parseFloat(outReceiveMoneyTotal).toFixed(2);
          outInterestTotal=outInterestTotal && parseFloat(outInterestTotal).toFixed(2)
    ...
    handle_delete(row, index, cg) {
      ...
      datas &&
        ...
        ticketAmountTotal=ticketAmountTotal && (parseFloat(ticketAmountTotal)).toFixed(2);
        inPayMoneyTotal=inPayMoneyTotal && (parseFloat(inPayMoneyTotal)).toFixed(2);
        inInterestTotal=inInterestTotal && (parseFloat(inInterestTotal)).toFixed(2);
        outReceiveMoneyTotal=outReceiveMoneyTotal && (parseFloat(outReceiveMoneyTotal)).toFixed(2);
        outInterestTotal=outInterestTotal && (parseFloat(outInterestTotal)).toFixed(2);
           this.billInStockFlow = Object.assign(billInStockFlow, {
            ticketAmountTotal,
...
.newMoveLibrary header > div {
  width: 33%;
}
.newMoveLibrary header > div:last-child {
  width: 34%;
}
.newMoveLibrary header div > label {
  display: block;
  margin-bottom: 24px;
}
.newMoveLibrary header div > label > span {
  margin-right: 10px;
  font-weight: 600;
}
.newMoveLibrary .infoHead {
  border-bottom: 2px solid #e6e9f0;
}
.newMoveLibrary .infoHead div {
  padding-bottom: 8px;
}
.newMoveLibrary .infoHead .fl {
  padding-left: 12px;
  position: relative;
  color: #5376ff;
}
.newMoveLibrary .infoHead .fl i {
  position: absolute;
  left: 0;
  bottom: -2px;
  width: 80px;
...
.newMoveLibrary .section .headClass {
  color: #a3adc8;
}
.newMoveLibrary .section .addRow {
  margin-top: 25px;
  color: #5376ff;
  display: inline-block;
  cursor: pointer;
}
.newMoveLibrary .section .addRow i {
  margin-right: 8px;
}
.newMoveLibrary .section .total {
  margin-top: 27px;
}
.newMoveLibrary .section .total .top {
  margin-bottom: 16px;
  color: #596689;
}
.newMoveLibrary .section .total li {
...
.newMoveLibrary .section .total li .txt {
  line-height: 26px;
  font-size: 12px;
  color: rgba(163, 173, 200, 1);
}
.newMoveLibrary .section .total li .num {
  line-height: 26px;
  font-size: 18px;
  font-weight: 600;
